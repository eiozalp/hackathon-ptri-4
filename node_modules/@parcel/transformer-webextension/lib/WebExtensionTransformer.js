...
const DEP_LOCS = [['icons'], ['browser_action', 'default_icon'], ['browser_action', 'default_popup'], ['browser_action', 'theme_actions', 'light'], ['browser_action', 'theme_actions', 'dark'], ['page_action', 'default_icon'], ['page_action', 'default_popup'], ['background', 'scripts'], ['chrome_url_overrides'], ['devtools_page'], ['options_ui', 'page'], ['sidebar_action', 'default_icon'], ['sidebar_action', 'default_panel'], ['storage', 'managed_schema'], ['theme', 'images', 'theme_frame'], ['theme', 'images', 'additional_backgrounds'], ['user_scripts', 'api_script']];
...
async function collectDependencies(asset, program, ptrs, hot) {
  ...
  if (program.default_locale) {
    const locales = (0, _path().join)((0, _path().dirname)(filePath), '_locales');
    let err = !(await fs.exists(locales)) ? 'key' : !(await fs.exists((0, _path().join)(locales, program.default_locale))) ? 'value' : null;
        ...
        diagnostic: [{
          ...
          codeFrame: {
            codeHighlights: [{ ...(0, _diagnostic().getJSONSourceLocation)(ptrs['/default_locale'], err),
              message: `Localization directory${err == 'value' ? ' for ' + program.default_locale : ''} does not exist: ${(0, _path().relative)((0, _path().dirname)(filePath), (0, _path().join)(locales, program.default_locale))}`
            }]
          }
        }]
      });
  ...
  if (program.dictionaries) {
    for (const dict in program.dictionaries) {
      ...
      if ((0, _path().extname)(dictFile) != '.dic') {
        throw new (_diagnostic().default)({
          diagnostic: [{
            message: 'Invalid Web Extension manifest',
            origin: '@parcel/transformer-webextension',
  ...
  if (program.web_accessible_resources) {
    for (let i = 0; i < program.web_accessible_resources.length; ++i) {
      ...
      const globQuery = (0, _path().join)((0, _path().dirname)(filePath), program.web_accessible_resources[i]);

      for (const fp of await (0, _utils().glob)(globQuery, fs, {})) {
        asset.addURLDependency((0, _path().relative)((0, _path().dirname)(filePath), fp), {
          isEntry: true,
          loc: {
            filePath,
  ...
  for (const loc of DEP_LOCS) {
    ...
    if (typeof obj == 'string') parent[lastLoc] = asset.addURLDependency(obj, {
      ...
      pipeline: (0, _path().extname)(obj) == '.json' ? 'url' : undefined
    });else {
      for (const k of Object.keys(obj)) {
        obj[k] = asset.addURLDependency(obj[k], {
          ...
          pipeline: (0, _path().extname)(obj[k]) == '.json' ? 'url' : undefined
        });
      }
    }
  }
